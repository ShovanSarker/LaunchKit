...
# =============================================================================
# LaunchKit - Celery Scheduler Runner (Development)
# =============================================================================
...
start_scheduler() {
    print_message "Starting Celery scheduler service..."
    ...
    print_success "Celery scheduler service started!"
    print_message "Scheduler will manage periodic tasks"
    
    # Show logs
    print_message "Showing scheduler logs (Ctrl+C to stop):"
    docker compose logs -f scheduler
}

# Stop scheduler service
stop_scheduler() {
    print_message "Stopping Celery scheduler service..."
    cd "${PROJECT_ROOT}/docker"
    docker compose stop scheduler
    print_success "Celery scheduler service stopped!"
}

# Restart scheduler service
restart_scheduler() {
    print_message "Restarting Celery scheduler service..."
    stop_scheduler
    sleep 2
    start_scheduler
}
...
# Show status
show_status() {
    print_message "Scheduler service status:"
    cd "${PROJECT_ROOT}/docker"
...
main() {
    case "${1:-start}" in
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "Commands:"
            echo "  start   - Start the Celery scheduler service (default)"
            echo "  stop    - Stop the Celery scheduler service"
            echo "  restart - Restart the Celery scheduler service"
            echo "  status  - Show scheduler service status"
            echo "  logs    - Show scheduler service logs"
