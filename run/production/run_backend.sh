...
# =============================================================================
# LaunchKit - Backend Service Runner (Production)
# =============================================================================
...
check_database() {
    ...
    if ! docker ps --format "{{.Names}}" | grep -q "${PROJECT_SLUG:-launchkit}_db"; then
        print_warning "Database container is not running. Starting database..."
        cd "${PROJECT_ROOT}/docker" && docker compose up -d db
        sleep 10
    fi
}
...
start_backend() {
    ...
    print_message "API will be available at: https://api.${DOMAIN:-localhost}"
    print_message "API Documentation: https://api.${DOMAIN:-localhost}/api/docs/"
...
main() {
    case "${1:-start}" in
        ...
        "createsuperuser")
            ...
            ;;
        "help"|"-h"|"--help")
            echo "Usage: $0 [start|stop|restart|status|logs|migrate|collectstatic|createsuperuser|help]"
            echo ""
            echo "Commands:"
